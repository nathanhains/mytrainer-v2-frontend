...
const AddedExerciseCard = ({exercise, i, updateWorkoutForm, workoutFormData}) =>{
        ...
        return <>
            <span class="gray addedWorkoutExerciseName" key={i}> {exercise.attributes.name} <i onClick={() => updateWorkoutForm({...workoutFormData, exercises: workoutFormData.exercises.filter(e=> e.id !== exercise.id)})} class="settings formDisplayName fa fa-trash"></i></span> 
